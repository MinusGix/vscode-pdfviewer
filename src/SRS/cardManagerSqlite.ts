/**
 * CardManagerSqlite - SQLite-backed card management
 *
 * Uses CardReviewStateSqlite for FSRS state persistence.
 */
...
import { CardReviewStateSqlite } from './cardReviewStateSqlite';
...
export class CardManagerSqlite implements vscode.Disposable {
    private static instance: CardManagerSqlite;
    private cardsByFile: Map<string, MdCard[]>;
    private disposables: vscode.Disposable[];
    private initialized: boolean;
    private readonly _onDidUpdateCards =
        new vscode.EventEmitter<CardUpdateEvent>();
    public readonly onDidUpdateCards = this._onDidUpdateCards.event;
    private reviewState: CardReviewStateSqlite;
    ...
    private constructor() {
        ...
        // Initialize SQLite-backed review state
        this.reviewState = new CardReviewStateSqlite();
            ...
            async (e) => {
                ...
                for (const folder of e.added) {
                    ...
                    for (const [uri, parseResult] of results) {
                        const cardsWithIds = parseResult.cards.map((card) =>
    ...
    public static getInstance(): CardManagerSqlite {
        if (!CardManagerSqlite.instance) {
            CardManagerSqlite.instance = new CardManagerSqlite();
        }
        return CardManagerSqlite.instance;
    }
    ...
    public async initialize(): Promise<void> {
        ...
        try {
            // Initialize the SQLite-backed review state
            await this.reviewState.initialize();
            ...
            console.error('Failed to initialize CardManagerSqlite:', error);
        } finally {
    ...
    public getFilesWithCards(): vscode.Uri[] {
        return Array.from(this.cardsByFile.keys()).map((uri) =>
    ...
    public async insertCardTemplateTesting(withId: boolean = true): Promise<void> {
        ...
        const cardId = withId
            ? generateCardId({ front: 'New Card', back: '', tags: [], type: 'basic' })
            : undefined;
        const template = `:::card
${cardId ? `id: ${cardId}\n` : ''}type: basic
front: |
...
`;
    ...
    public async disableCurrentFile(): Promise<void> {
        ...
        const isAlreadyDisabled = lines.some(
            (line) =>
    ...
    public async enableCurrentFile(): Promise<void> {
        ...
        for (let i = 0; i < Math.min(10, lines.length); i++) {
            if (
                lines[i].trim().match(/^<!--\s*lattice:disabled\s*-->$/i) ||
    ...
    public getDueCards(now?: Date): MdCard[] {
        ...
        const dueCards = this.getAllCards().filter(
            (card) => !card.disabled && card.id && dueCardIds.includes(card.id)
        );

        // Determine ordering mode
        const config = vscode.workspace.getConfiguration('lattice.cards');
        const mode = config.get<
            'dueTime' | 'random-daily-global' | 'random-daily-per-file'
        >('reviewOrder', 'random-daily-per-file');
        ...
        if (mode === 'random-daily-global') {
            return [...dueCards].sort(
                (a, b) => this.hashForCard(seed, a) - this.hashForCard(seed, b)
            );
        }
        ...
        files.sort(
            (a, b) =>
                this.hashForString(seed + '|file|' + a) -
                this.hashForString(seed + '|file|' + b)
        );
    ...
    public dispose(): void {
        ...
        this.initialized = false;
        CardManagerSqlite.instance = undefined!;
    }

    private updateStatusBar(): void {
        const dueCards = this.getDueCards();
        const threshold: number = vscode.workspace
            .getConfiguration('lattice.cards')
            .get('statusBarColorThreshold', 5);
        ...
        if (dueCards.length > 0) {
            ...
            if (threshold !== -1) {
                ...
                const dueRatio =
                    dueCards.length / Math.max(threshold, totalCards * 0.05); // Use whichever is larger: threshold or 5% of total cards
