...
function updateCursorPosition() {
  try {
    ...
    // Try to find the element at this offset using position mapping
    const targetElement = findElementAtOffset(textOffset);

    if (targetElement && positionIndex.length > 0) {
      // We have a mapped element, try to position cursor within it
      const range = document.createRange();

      // Find the closest text node within the target element
      ...
      while (textNode) {
        ...
        if (currentOffset + nodeLength >= textOffset) {
          const nodeOffset = Math.min(textOffset - currentOffset, nodeLength);
...
content.addEventListener('click', e => {
  try {
    ...
    if (range) {
      // Find the element with data-sourcepos that contains this range
      let element = range.startContainer;
      while (element && element.nodeType !== Node.ELEMENT_NODE) {
        element = element.parentNode;
      }

      // Look for the closest element with data-sourcepos
      while (element && !element.dataset.sourcepos) {
        element = element.parentNode;
      }

      if (element && element.dataset.sourcepos) {
        const [start, end] = element.dataset.sourcepos.split(':').map(Number);
        ...
        while (textNode) {
          if (textNode === range.startContainer) {
            clickOffset = currentOffset + range.startOffset;
            break;
          }
